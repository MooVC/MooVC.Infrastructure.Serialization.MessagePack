    ...
    : Base
{
    ...
    protected override async Task<T> PerformDeserializeAsync<T>(Stream source, CancellationToken? cancellationToken = default)
    {
        T? result = await PerformDeserializeAsync<T>(source, cancellationToken.GetValueOrDefault())
            .ConfigureAwait(false);

        if (result is null)
        {
            throw new MessagePackSerializationException(Format(SerializerPerformDeserializeAsyncFailure, typeof(T).AssemblyQualifiedName));
        }

        return result;
    }

    protected abstract ValueTask<T> PerformDeserializeAsync<T>(Stream source, CancellationToken cancellationToken)
        where T : notnull;
}