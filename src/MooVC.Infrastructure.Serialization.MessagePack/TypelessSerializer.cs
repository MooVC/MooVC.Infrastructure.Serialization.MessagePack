    ...
    : Serializer
{
    public TypelessSerializer(
        ...
        : base(compressor: compressor, configure: configure, options: MessagePackSerializer.Typeless.DefaultOptions)
    {
    }
    ...
    protected override async ValueTask<T> PerformDeserializeAsync<T>(Stream source, CancellationToken cancellationToken)
    {
        object? instance = await MessagePackSerializer
            .Typeless
            .DeserializeAsync(source, options: Options, cancellationToken: cancellationToken)
